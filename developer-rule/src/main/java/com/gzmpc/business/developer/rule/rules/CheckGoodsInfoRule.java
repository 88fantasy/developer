...
@RuleProperties(input = "goodsid, cpid", output = "cutPriceGoodsFlag", tags = {"降价品种", "审批和受理证明", "货品状态"})
...
public class CheckGoodsInfoRule {
    ...
    public boolean isUse(@Fact("goodsid") long goodsid, @Fact("cpid") long cpid, Facts facts) {
		...
		if(pubGoods != null && pubGoods.isUsestatus()) {
			...
			if(sysCp != null) {
				HashMap retcodeMap = zxChkUseStatusMapper.SelectRetcode(sysCp.getOpid(), 0);
				if(retcodeMap != null) {
					...
					if(actmsg != null && !actmsg.equals("")) {
						factsUtil.setMessage(facts, RuleConstants.RULE_TIPS_MESSAGE_KEY, actmsg+" ");
					}
				}
			}
		}
